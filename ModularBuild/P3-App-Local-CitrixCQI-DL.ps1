...
function Install {
    ...
    if ($DLEXE -like "*.zip") {
		...
		Expand-Archive -Path "$($DownloadFolder + $DLEXE)" -DestinationPath "$($DownloadFolder + $Application)" -Force -Verbose
    }
    ...
    #}
		...
		Start-Process "msiexec" -ArgumentList "/i $($DownloadFolder + $Application + "\" + $InstallerName) $Arguments" -Wait -PassThru
    #}
}
